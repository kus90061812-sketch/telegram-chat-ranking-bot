...
`.채팅순위`에는 상금표가 함께 표시되고, `.나`에는 현재 주간 순위 기준 예상 상금이 표시됩니다. 상금은 웹 관리자에서 언제든 바꿀 수 있습니다.
...
- 명령어, 이모티콘만 있는 메시지, 2글자 미만 메시지 제외
...
| `.나` | `{NAME}`, `{DAILY_COUNT}`, `{DAILY_RANK}`, `{DAILY_GAP}`, `{WEEKLY_COUNT}`, `{WEEKLY_RANK}`, `{WEEKLY_GAP}`, `{PRIZE}`, `{DAY_DATE}`, `{WEEK_DATE}` |
