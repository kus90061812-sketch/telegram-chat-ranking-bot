...
from chat_rank_bot.storage import ChatSettings, Storage
...
class StorageTests(unittest.TestCase):
    ...
    def test_chat_settings_can_be_saved_and_reloaded(self) -> None:
        now = datetime(2026, 8, 22, tzinfo=timezone.utc)
        self.storage.register_chat(-100, "AXIS 소통방", now)
        self.assertEqual(self.storage.list_chats()[0].title, "AXIS 소통방")
        custom = ChatSettings(
            chat_id=-100,
        ...
        self.storage.save_chat_settings(custom)
        loaded = self.storage.get_chat_settings(-100)
