...
class FormattingTests(unittest.TestCase):
    ...
    def test_personal_message_matches_vertical_prize_layout(self) -> None:
        ...
        self.assertIn("🪶 <b>내 채팅 기록</b>", result)
        self.assertIn("📅 오늘: <b>151회</b>", result)
        self.assertIn("📆 이번 주: <b>891회</b>", result)
        self.assertIn("5글자 이상 집계 / 초성 집계 X", result)
        self.assertIn("<b>🥇1등 : 10만원</b>", result)
        self.assertIn("<b>▪4등 : 2만원</b>", result)
        self.assertIn("<b>▪▪▪▪▪▪ 포인트로 지급</b>", result)

    def test_person_outside_top_four_has_no_expected_prize(self) -> None:
        settings = ChatSettings(
            chat_id=-100,
            personal_template="{NAME}: {PRIZE_BOLD}",
        )
        result = personal_message(
            "라온",
        ...
        self.assertIn("라온: <b>순위권 밖</b>", result)
